...
def stg_paie_bulletins(
    ...
    records = [
        (
            ...
            # Gross salary
            b["salaire_brut"].get("base_salary"),
            b["salaire_brut"].get("anciennete", 0),
            b["salaire_brut"].get("prime_rendement"),
            b["salaire_brut"].get("prime_panier"),
            b["salaire_brut"].get("prime_transport"),
            b["salaire_brut"].get("heures_sup_amount"),
            b["salaire_brut"].get("heures_sup_hours"),
            b["salaire_brut"].get("autres_primes"),
            b["salaire_brut"].get("total_brut"),
            # Deductions
            ...
            # Employer charges
            b["charges_patronales"].get("cotisation_patronale_cnas"),
            b["charges_patronales"].get("cotisation_retraite"),
            b["charges_patronales"].get("accident_travail"),
            b["charges_patronales"].get("total_charges_patronales"),
            # Net — RIB excluded here intentionally
            b["net"].get("net_a_payer"),
            b["net"].get("mode_paiement", ""),
            # Time
            int(b["temps_travaille"].get("jours_travailles", 0)),
            int(b["temps_travaille"].get("jours_absence", 0)),
            int(b["temps_travaille"].get("jours_conge", 0)),
            int(b["temps_travaille"].get("jours_maladie", 0)),
            float(b["temps_travaille"].get("heures_normales", 0)),
            b["temps_travaille"].get("heures_sup"),
            batch_id,
        )
        for b in bulletins
